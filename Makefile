...
add-copyright-license-headers:
	...
	docker run --rm -v $(shell pwd)/multi_agent_jarvis:/workspace ghcr.io/google/addlicense:latest -c "CNOE" -l apache -s=only -v /workspace
...
run-acp: setup-venv build install
	...
	. .venv/bin/activate && set -a && . .env && set +a && wfsm deploy -m ./agent_argocd/protocol_bindings/acp_server/agent.json -e .env --dryRun=false
